...
export function normalizeCpuTitle(title: string, mpn?: string, manufacturer?: string): string {
  ...
  if (/\bAMD\b/i.test(cleanTitle) || manufacturer?.toUpperCase() === "AMD") {
    brand = "AMD";
  } else if (/\bINTEL\b/i.test(cleanTitle) || manufacturer?.toUpperCase() === "INTEL") {
    brand = "Intel";
  }
  ...
  if (cores === 0) {
    const coresMatch = cleanTitle.match(/(\d+)\s*(?:Cores?|Núcleos?|Nucleos?|-?Core)/i);
    if (coresMatch) {
  ...
  if (threads === 0) {
    const threadsMatch = cleanTitle.match(/(\d+)\s*(?:Threads?|Hilos?|-?Thread)/i);
    if (threadsMatch) {
  ...
  if (brand && family && model) {
    ...
    } else if (brand === "AMD" && family.startsWith("A") && family.length <= 3) {
      result = `${brand} ${family}-${model}`;
    } else {
      result = `${brand} ${family} ${model}`;
    }
...
function buildLookupKey(brand: string, family: string, model: string): string {
  if (brand === "AMD") {
    ...
    if (family.startsWith("Core")) {
      ...
      if (familyParts.length === 2 && familyParts[0] === "Core" && familyParts[1].startsWith("i")) {
