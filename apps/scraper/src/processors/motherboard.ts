...
export const MotherboardProcessor = {
  normalize(rawSpecs: Record<string, string>): MotherboardSpecs {
    return {
      ...
      memory_slots:
        rawSpecs.memory_slots || rawSpecs.slots_memorias || rawSpecs["slots memorias"] || "",
      memory_channels:
        rawSpecs.memory_channels || rawSpecs.canales_memoria || rawSpecs["canales memoria"] || "",
      form_factor: rawSpecs.form_factor || rawSpecs.formato || "",
      rgb_support: parseList(
        rawSpecs.rgb_support || rawSpecs.soporte_rgb || rawSpecs["soporte rgb"] || "",
      ),
      video_ports: parseList(
        rawSpecs.video_ports || rawSpecs.puertos_de_video || rawSpecs["puertos de video"] || "",
      ),
      ...
      sli_support: parseBoolean(
        rawSpecs.sli_support || rawSpecs.soporte_sli || rawSpecs["soporte sli"] || "",
      ),
      ...
      raid_support: parseBoolean(
        rawSpecs.raid_support || rawSpecs.soporte_raid || rawSpecs["soporte raid"] || "",
      ),
