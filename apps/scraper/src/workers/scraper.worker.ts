...
import type { CrawlerType, ScraperJobData } from "@/queues";
...
async function handlePcExpressJob(job: ScraperJobData): Promise<JobResult> {
  const startTime = Date.now();
...
async function handleSpDigitalJob(job: ScraperJobData): Promise<JobResult> {
  const startTime = Date.now();
...
self.onmessage = async (event: MessageEvent) => {
  const job = event.data as ScraperJobData;
