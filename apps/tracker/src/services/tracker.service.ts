...
type Listing = Pick<
	Tables<"listings">,
	"id" | "url" | "price_cash" | "price_normal" | "stock_quantity"
> & {
...
export class TrackerService {
	...
	async trackBatch(limit = 0) {
		...
		const myShopListings = listings.filter(
			(l) => l.url.includes("myshop.cl") && l.product?.mpn,
		);
		...
		this.logger.info(
			`MyShop listings with MPN: ${myShopListings.length}, Other/No-MPN: ${otherListings.length}`,
		);
		...
		if (otherListings.length > 0) {
			const individualResult =
				await this.processIndividualListings(otherListings);
	...
	private async processMyShopBatch(listings: Listing[]) {
		this.logger.info(
			`Processing ${listings.length} MyShop listings via MPN batch...`,
		);
		...
		try {
			...
			for (const listing of listings) {
				...
				if (!result) {
					this.logger.warn(
						`No result found for MPN: ${mpn} (listing: ${listing.id})`,
					);
					errors++;
					continue;
				}
			...
			listings.map((listing) => {
				...
				if (!tracker) {
					return Promise.reject(
						new Error(`No tracker found for URL: ${listing.url}`),
					);
				}

				const limiter =
					tracker instanceof SpDigitalTracker
						? this.heavyLimit
						: this.lightLimit;
				...
				return limiter(async () => {
					...
					if (tracker instanceof MyShopTracker && listing.product?.mpn) {
						const result = await this.myShopTracker.trackByMpn(
							listing.product.mpn,
						);
	...
	private async updateListing(listing: Listing, result: TrackerResult) {
		...
		const hasPriceChanged =
			oldPriceCash !== result.price ||
			(result.priceNormal && oldPriceNormal !== result.priceNormal);
