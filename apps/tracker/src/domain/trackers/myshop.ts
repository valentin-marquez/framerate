...
export class MyShopTracker extends BaseTracker {
  ...
  async track(url: string): Promise<TrackerResult> {
    ...
    this.logger.warn(`[MyShop] Cannot track by URL alone: ${url}. Use trackByMpn instead.`);
  ...
  async trackByMpn(mpn: string): Promise<TrackerResult> {
    ...
    this.logger.info(`[MyShop] MPN ${cleanMpn} not in global cache, searching categories...`);
    try {
    ...
    for (const mpn of mpns) {
      ...
      if (!product) {
        try {
          ...
          this.logger.error(`[MyShop] Error looking up MPN ${cleanMpn} in batch:`, err);
        }
      }
  ...
  private async ensureCache(): Promise<void> {
    ...
    if (this.isRefreshing && this.refreshPromise) {
      this.logger.info("[MyShop] Cache refresh already in progress, waiting...");
   ...
   */
  private async fetchFamilyCompletely(category: string, familyId: string): Promise<void> {
    let page = 1;
    ...
    while (hasMorePages && page <= this.MAX_PAGES_PER_FAMILY) {
      try {
        ...
        if (!data?.resultado?.items || data.resultado.items.length === 0) {
          this.logger.info(`[MyShop] Family ${familyId} completed at page ${page - 1}`);
          hasMorePages = false;
          break;
        }
        ...
        this.logger.error(`[MyShop] Error fetching family ${familyId} page ${page}:`, error);
        hasMorePages = false;
      }
    }

    this.logger.info(`[MyShop] Family ${familyId} done: ${page - 1} pages processed`);
  }

  private async fetchApiPageOnce(idFamilia: string, page: number): Promise<MyShopApiResponse> {
    const payload = {
      tipo: "3",
      page: String(page),
      idFamilia: idFamilia,
    ...
    try {
      ...
      if (!response.ok) {
        ...
        if (response.status === 429) {
          throw new Error(`[MyShop] API Status: 429. Retry-After:${retryAfter ?? ""}`);
        }
      ...
      if (err instanceof Error && err.name === "AbortError") {
        throw new Error(`[MyShop] fetch timeout after ${this.FETCH_TIMEOUT_MS}ms`);
      }
      throw err;
    } finally {
  ...
  private async fetchApiPage(idFamilia: string, page: number): Promise<MyShopApiResponse | undefined> {
    for (let attempt = 0; attempt <= this.MAX_RETRIES; attempt++) {
      try {
        return await this.requestLimiter(() => this.fetchApiPageOnce(idFamilia, page));
      } catch (err) {
        ...
        if (message.includes("403")) {
          ...
          const backoff = Math.min(this.RETRY_BASE_MS * 2 ** (attempt + 2) + Math.random() * 1000, 20_000);
          this.logger.warn(
        ...
        if (attempt === this.MAX_RETRIES) {
          this.logger.error(`[MyShop] Failed after ${this.MAX_RETRIES} attempts:`, err);
          return undefined;
        }

        const backoff = Math.min(this.RETRY_BASE_MS * 2 ** attempt + Math.random() * 500, 10_000);
        this.logger.warn(`[MyShop] Retry in ${backoff}ms (attempt ${attempt + 1}/${this.MAX_RETRIES})`);
        await this.delay(backoff);
      }
    }
  ...
  private async findProductByMpnFallback(cleanMpn: string): Promise<MyShopItem | undefined> {
    ...
    for (const familyIds of Object.values(MYSHOP_CATEGORIES)) {
      for (const familyId of familyIds) {
        try {
          ...
          this.logger.error(`[MyShop] Error searching family ${familyId} for ${cleanMpn}:`, err);
        }
      }
    }
  ...
  private async fetchProductByMpn(idFamilia: string, cleanMpn: string): Promise<MyShopItem | undefined> {
    let page = 1;
    let pagesChecked = 0;
    ...
    while (pagesChecked < this.MAX_PAGES_PER_FAMILY) {
      ...
      try {
        ...
        if (!data?.resultado?.items || data.resultado.items.length === 0) return undefined;

        for (const item of data.resultado.items) {
          if (item.partno && item.partno.trim().toUpperCase() === cleanMpn) return item;
        }
        ...
        this.logger.error(`[MyShop] Error fetching family ${idFamilia} page ${page}:`, err);
        return undefined;
      }
    }
  ...
  private productToTrackerResult(product: MyShopItem): TrackerResult {
    const isAgotadoLabel = typeof product.label === "string" && product.label.toLowerCase().includes("agotado");

    const hasStock = (product.stock_total > 0 || product.disponibleInternet) && !isAgotadoLabel;

    return {
      price: product.precio,
      priceNormal: product.precio_tarjeta > product.precio ? product.precio_tarjeta : product.precio_normal,
      stock: hasStock,
