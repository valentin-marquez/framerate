...
export const trackRoutes = new Elysia({ prefix: "/track" })
	...
	.get(
		...
		},
		{
			detail: {
				...
				description:
					"Returns MyShop cache statistics including size, age, and request counts",
				tags: ["Cache"],
			},
		},
	)
	...
	.post(
		...
		},
		{
			detail: {
				...
				description:
					"Manually trigger a full refresh of the MyShop product cache",
				tags: ["Cache"],
			},
		},
	)
	...
	.get(
		...
		({ trackerService }) => {
			const stats = trackerService.getCacheStats();
			const isHealthy =
				stats.size > 0 && stats.age < stats.ttl && !stats.isRefreshing;
