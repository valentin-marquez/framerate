...
import { CENTRAL_GAMER_CATEGORIES } from "@/crawlers/central-gamer";
import { Logger } from "@/lib/logger";

const app = new Hono();
const logger = new Logger("CentralGamerRoute");
...
app.post("/crawl", async (c) => {
  try {
    ...
    const categoryMessage = isAll
      ? `todas las categorías (${Object.keys(CENTRAL_GAMER_CATEGORIES).join(", ")})`
      : `categoría "${category}"`;
