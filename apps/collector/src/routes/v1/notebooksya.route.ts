...
import { NOTEBOOKSYA_CATEGORIES } from "@/crawlers/notebooksya";
import { Logger } from "@/lib/logger";

const app = new Hono();
const logger = new Logger("NotebooksYaRoute");
...
app.post("/crawl", async (c) => {
  try {
    ...
    const categoryMessage = isAll
      ? `todas las categorías (${Object.keys(NOTEBOOKSYA_CATEGORIES).join(", ")})`
      : `categoría "${category}"`;
