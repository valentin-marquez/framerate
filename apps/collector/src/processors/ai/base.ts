...
export abstract class BaseIAExtractor<T> {
  ...
  constructor() {
    ...
    const mode = Bun.env.AI_MODE || (Bun.env.NODE_ENV === "development" ? "groq" : "deepseek");
    const isGroq = mode === "groq";

    const apiKey = isGroq ? Bun.env.GROQ_API_KEY : Bun.env.DEEPSEEK_API_KEY;
    const baseURL = isGroq ? "https://api.groq.com/openai/v1" : "https://api.deepseek.com";
  ...
  async extract(mpn: string, text: string, context?: any): Promise<T | null> {
    ...
    try {
      ...
      // 2. Call LLM
      this.logger.info(`Cache MISS for MPN: ${mpn}. Calling LLM (${this.model})...`);
      ...
      if (specs) {
        ...
        const { error: insertError } = await (supabase as any).from("cached_specs_extractions").insert({
          mpn,
          specs: specs,
        });

        if (insertError) {
          this.logger.error(`Error caching specs for MPN ${mpn}:`, insertError.message);
        } else {
