...
const GPU_BRANDS: Record<string, { name: string; keywords: string[]; lines: Record<string, string[]> }> = {
  ASUS: {
    name: "ASUS",
    keywords: ["ASUS"],
    lines: {
...
function extractGpuBrandLine(title: string, mpn?: string): BrandModel | null {
  ...
  for (const [_key, brand] of Object.entries(GPU_BRANDS)) {
    ...
    const brandMatch = brand.keywords.some((kw) => titleUpper.includes(kw) || mpnUpper.includes(kw));

    if (brandMatch) {
      // Try to detect product line
      for (const [lineName, lineKeywords] of Object.entries(brand.lines)) {
        const lineMatch = lineKeywords.some((kw) => titleUpper.includes(kw) || mpnUpper.includes(kw));
        if (lineMatch) {
  ...
  if (!chip) {
    ...
    if (manufacturer) {
      ...
      if (!cleanTitle.toUpperCase().includes(manuUpper)) {
        ...
        const properManu = manufacturer.charAt(0).toUpperCase() + manufacturer.slice(1).toLowerCase();
    ...
    if (chip.series === "GT") {
      ...
      if (chip.variant) {
        ...
        const formattedVariant =
          chip.variant === "TI" ? "Ti" : chip.variant.charAt(0) + chip.variant.slice(1).toLowerCase();
