...
export function normalizeCaseFanTitle(title: string, mpn?: string, manufacturer?: string): string {
  let cleaned = cleanHtmlEntities(title);

  // 1. Detectar y normalizar Marca
  let brand = "";
  const brandMatch = BRANDS.find((b) => cleaned.toLowerCase().includes(b.toLowerCase()));
  if (brandMatch) {
    brand = brandMatch;
  } else if (manufacturer) {
    const mfrMatch = BRANDS.find((b) => manufacturer.toLowerCase().includes(b.toLowerCase()));
    if (mfrMatch) {
      brand = mfrMatch;
    } else {
  ...
  cleaned = cleaned.replace(/(\d+)\s*mm\s*x\s*\d+\s*mm(?:\s*x\s*\d+\s*mm)?/gi, "$1mm");
  ...
  if (brand.toLowerCase() === "gamemax" && mpn) {
    ...
    if (upperMpn.includes("FN12A-S8I-R") && !cleaned.toUpperCase().includes("FN12A")) {
      cleaned = cleaned.replace(/120mm/gi, "").trim();
      cleaned += " FN12A-S8I-R";
      if (upperMpn.includes("BLACK") && !cleaned.includes("Black") && !colors.includes("Black")) {
        cleaned += " Black";
      }
    } else if (upperMpn.includes("FN-12RAINBOW-C9") && !cleaned.toUpperCase().includes("RAINBOW-C9")) {
      cleaned = cleaned.replace(/120mm/gi, "").trim();
      cleaned += " FN12 Rainbow C9";
    } else if (upperMpn.includes("FN-12RAINBOW-Q-INFINITY") && !cleaned.toUpperCase().includes("Q-INFINITY")) {
      cleaned = cleaned.replace(/120mm/gi, "").trim();
