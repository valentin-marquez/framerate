...
function toTitleCase(str: string): string {
	return str.replace(/\w\S*/g, (txt) => {
		...
		if (
			/^(DDR\d|SDRAM|RGB|ARGB|CL\d+|PC\d+|XPG|HP|OC|LED|DIMM|UDIMM|SODIMM|CUDIMM)$/i.test(
				txt,
			)
		) {
...
export function normalizeRamTitle(
	title: string,
	_mpn?: string | null,
	manufacturer?: string,
): string {
	...
	cleanTitle = cleanTitle.replace(
		/\b400 PC3200 512MB PC320\b/gi,
		"PC3200 512MB",
	);
	cleanTitle = cleanTitle.replace(
		/\b266 PC2100 512MB PC210\b/gi,
		"PC2100 512MB",
	);
	cleanTitle = cleanTitle.replace(/\b266 PC2100 1GB PC210\b/gi, "PC2100 1GB");
	cleanTitle = cleanTitle.replace(
		/\b2666 PC21300 8GB DDR4 PC213\b/gi,
		"PC4-21300 8GB DDR4",
	);
	cleanTitle = cleanTitle.replace(
		/\b333 PC2700 512MB PC270\b/gi,
		"PC2700 512MB",
	);
	cleanTitle = cleanTitle.replace(
	...
	if (manufacturer) {
		const knownBrand = BRANDS.find(
			(b) => b.toLowerCase() === manufacturer.toLowerCase(),
		);
	...
	if (!brand) {
		...
		const matches = BRANDS.filter((b) =>
			new RegExp(`\\b${b}\\b`, "i").test(cleanTitle),
		);
		if (matches.length > 0) {
	...
	// Eliminar Tipo
	if (type)
		modelPart = modelPart.replace(new RegExp(`\\b${type}\\b`, "gi"), "");
	...
	if (speed) {
		...
		if (speedVal.startsWith("PC")) {
			...
			modelPart = modelPart.replace(
				new RegExp(`\\b${speedVal}\\s*(?:MT/s|MHz|HZ)\\b`, "gi"),
				"",
			);
			modelPart = modelPart.replace(new RegExp(`\\b${speedVal}\\b`, "gi"), "");
		}
