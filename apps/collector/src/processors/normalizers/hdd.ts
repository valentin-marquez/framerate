...
function detectWdHddLineFromMpn(mpn: string): string | null {
	...
	if (
		mpnUpper.match(/WD\d+EZAZ/) ||
		mpnUpper.match(/WD\d+EZEX/) ||
		mpnUpper.match(/WD\d+EARZ/)
	)
		return "Blue";
	...
	if (mpnUpper.match(/WD\d+FZBX/) || mpnUpper.match(/WD\d+FZEX/))
		return "Black";
	...
	if (mpnUpper.match(/WD\d+PURZ/) || mpnUpper.match(/WD\d+PURX/))
		return "Purple";
...
function extractHddPurpose(title: string): string | null {
	...
	if (
		titleUpper.includes("VIDEOVIGILANCIA") ||
		titleUpper.includes("SURVEILLANCE")
	) {
		return "Vigilancia";
	}
 ...
 * Ejemplo: "Seagate SkyHawk AI 8TB 3.5" 7200RPM"
 */
export function normalizeHddTitle(
	title: string,
	mpn?: string,
	manufacturer?: string,
): string {
