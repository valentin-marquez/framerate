...
function detectWdHddLineFromMpn(mpn: string): string | null {
  ...
  if (mpnUpper.match(/WD\d+EZAZ/) || mpnUpper.match(/WD\d+EZEX/) || mpnUpper.match(/WD\d+EARZ/)) return "Blue";
 ...
 */
function extractHddBrandModel(title: string, mpn?: string, manufacturer?: string): BrandModel | null {
  const titleUpper = title.toUpperCase();
  const mfrUpper = manufacturer?.toUpperCase() || "";

  // Western Digital
  if (titleUpper.includes("WD") || titleUpper.includes("WESTERN DIGITAL") || mfrUpper.includes("WESTERN DIGITAL")) {
