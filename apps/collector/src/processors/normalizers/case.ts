...
function extractFormFactor(title: string): string | null {
	...
	const hasAtxCompatibility =
		/\bATX\s*[,/]\s*(?:MICRO|M-?ATX|MINI)/i.test(title) ||
		/\bATX\b.*\bMICRO-?ATX\b/i.test(title);
...
function cleanModel(title: string, brand: string): string {
	...
	cleaned = cleaned
		...
		.replace(
			/\b\d+[xX]\s*(?:FANS?|VENTILADOR(?:ES)?)\s*(?:ARGB|RGB)?\s*(?:\d+\s*MM)?\b/gi,
			"",
		)
		...
		.replace(
			/\bPANELES\s*FRONTALES\s*INTERCAMBIABLES\s*Y\s*VENTILADORES\b/gi,
			"",
		)
		.replace(/\bPANELES\s*INTERCAMBIABLES\b/gi, "")
		.replace(
			/\bCON\s*PANEL\s*DE\s*Y\s*AMPLIA\s*CAPACIDAD\s*DE\s*EXPANSIÓN\b/gi,
			"",
		)
	...
	if (brand === "Lian Li") {
		cleaned = cleaned
			.replace(/\bLANCOOL\b/gi, "Lancool")
			.replace(/\bO11\b/gi, "O11");
	}
	...
	cleaned = cleaned
		.replace(/\b[A-Z0-9]{2,}-[A-Z0-9]{4,}-[A-Z0-9]+\b/gi, "")
		.trim();
...
export function normalizeCaseTitle(
	title: string,
	mpn?: string,
	manufacturer?: string,
): string {
	...
	if (features.tg) {
		if (
			!model.toUpperCase().includes("TG") &&
			!model.toLowerCase().includes("glass")
		) {
			parts.push("TG");
		}
	}
	if (features.mesh) {
		if (
			!model.toLowerCase().includes("mesh") &&
			!model.toLowerCase().includes("airflow")
		) {
			parts.push("Mesh");
		}
	}
	if (features.rgb) {
		if (
			!model.toUpperCase().includes("ARGB") &&
			!model.toUpperCase().includes("RGB")
		) {
