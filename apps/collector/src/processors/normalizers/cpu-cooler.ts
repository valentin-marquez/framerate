...
function detectBrand(
	...
	for (const brand of Object.values(BRANDS)) {
		const found = brand.keywords.some(
...
function extractVariant(title: string): string | null {
	...
	else if (/\bPRO\b/.test(titleUpper) && !titleUpper.includes("A30 PRO"))
		variants.push("Pro");
	...
	const modelNumberMatch = titleUpper.match(
		/\b(\d{2,3})\s*(?:ARGB|ESSENTIAL|DIGITAL)\b/,
	);
	if (modelNumberMatch) {
...
function extractFeatures(title: string): string[] {
	...
	// Display features
	if (
		/\b\d+(\.\d+)?["'']\s*(?:IPS\s*)?LCD\b/.test(titleUpper) ||
		titleUpper.includes("IPS LCD")
	) {
		features.push("LCD");
...
export function normalizeCpuCoolerTitle(
	...
	if (
		size &&
		["240mm", "280mm", "360mm", "420mm"].includes(size) &&
		!coolerType
	) {
		coolerType = "aio";
	}
	...
	if (mpn) {
		workingTitle = workingTitle.replace(
			new RegExp(escapeRegExp(mpn.toUpperCase()), "g"),
			"",
		);
	}
	...
	if (productLine) {
		...
		for (const feat of features) {
			modelName = modelName.replace(
				new RegExp(escapeRegExp(feat.toUpperCase()), "g"),
				"",
			);
		}
		if (color) {
