...
const PRODUCT_LINES: Record<string, ProductLine[]> = {
  COOLER_MASTER: [
    { name: "MasterLiquid", keywords: ["MASTERLIQUID", "ML240", "ML280", "ML360"], type: "aio" },
    { name: "MasterAir", keywords: ["MASTERAIR"], type: "air" },
    { name: "Hyper", keywords: ["HYPER"], type: "air" },
  ],
  NZXT: [
    { name: "Kraken Elite", keywords: ["KRAKEN ELITE", "KR24E", "KR36E"], type: "aio" },
    { name: "Kraken Plus", keywords: ["KRAKEN PLUS", "KN24", "KN36"], type: "aio" },
  ...
  GAMEMAX: [
    ...
    { name: "Ice Surface", keywords: ["ICE SURFACE", "ICE-SURFACE"], type: "air" },
    { name: "Gamma", keywords: ["GAMMA"], type: "air" },
  ],
  GIGABYTE: [
    { name: "AORUS Waterforce X II", keywords: ["WATERFORCE X II"], type: "aio" },
    { name: "AORUS Waterforce", keywords: ["WATERFORCE"], type: "aio" },
  ],
  MSI: [
    { name: "MAG CoreLiquid", keywords: ["MAG CORELIQUID"], type: "aio" },
    { name: "MPG CoreLiquid", keywords: ["MPG CORELIQUID", "MPGCORELIQUID"], type: "aio" },
  ],
 ...
 */
function detectProductLine(
  title: string,
  brandKey: string | null,
): { name: string; type: "aio" | "air" } | null {
  const titleUpper = title.toUpperCase();
...
function extractColor(title: string): string | null {
  ...
  if (
    titleUpper.includes("WHITE") ||
    titleUpper.includes("BLANCO") ||
    titleUpper.includes("WITHE")
  ) {
    // typo
    return "White";
  }
 ...
 */
export function normalizeCpuCoolerTitle(
  title: string,
  mpn?: string,
  manufacturer?: string,
): string {
  ...
  if (productLine) {
    ...
    if (color) {
      modelName = modelName
        .replace(/WHITE|BLANCO|WITHE/gi, "")
        .replace(/BLACK|NEGRO|BK\b|BLK\b/gi, "");
    }
