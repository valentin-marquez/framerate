...
export function extractCertification(title: string): string | null {
  ...
  const standaloneMatch = titleUpper.match(/\b(TITANIUM|PLATINUM|PLATINIUM|PLATINO|GOLD|ORO|BRONZE|BRONCE)\b/i);
  if (standaloneMatch) {
...
export function extractModular(title: string): string | null {
  ...
  if (titleUpper.includes("FULL MODULAR") || titleUpper.includes("FULLY MODULAR")) {
  ...
  if (titleUpper.includes("MODULAR") && !titleUpper.includes("NON-MODULAR") && !titleUpper.includes("NO MODULAR")) {
    return "Modular";
  }
...
export function extractColor(title: string): string | null {
  ...
  if (titleUpper.includes("WHITE") || titleUpper.includes("BLANCO")) {
    ...
    const temp = titleUpper.replace(/80\s*(?:\+|PLUS)\s*WHITE/g, "").replace(/80\s*(?:\+|PLUS)\s*BLANCO/g, "");
...
export function extractBrand(title: string, manufacturer?: string): string | null {
  const titleUpper = title.toUpperCase();
...
function cleanSeriesModel(title: string, brand: string): string {
  ...
  if (cleaned === cleaned.toUpperCase() && cleaned.length > 3 && !/\d/.test(cleaned)) {
...
export function normalizePsuTitle(title: string, _mpn?: string, manufacturer?: string): string {
