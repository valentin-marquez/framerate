...
export function normalizeMotherboardTitle(title: string, _mpn?: string, manufacturer?: string): string {
  let normalized = cleanHtmlEntities(title);

  // 1. Eliminar prefijos comunes e información redundante
  normalized = normalized
    .replace(/^(COMBO\s+)?(PLACA MADRE|Motherboard|Placa Base|Mainboard)\s+/i, "")
