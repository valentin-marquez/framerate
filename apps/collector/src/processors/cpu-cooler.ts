...
function extractFanSizeFromTitle(title: string): string {
  ...
  if (aioMatch) {
    ...
    if (titleUpper.includes("LIQUIDA") || titleUpper.includes("WATERCOOLING") || titleUpper.includes("AIO")) {
      return `${size} mm`;
    }
  }
...
function extractIlluminationFromTitle(title: string): string {
  ...
  if (titleUpper.includes("ARGB") || titleUpper.includes("A-RGB") || titleUpper.includes("ADDRESSABLE")) {
    return "ARGB";
  }
...
function extractSocketsFromSpecs(rawSpecs: Record<string, string>): string[] {
  ...
  for (const [key, value] of Object.entries(rawSpecs)) {
    ...
    if (
      ...
      if (value.toLowerCase() === "sí" || value.toLowerCase() === "si" || value.toLowerCase() === "yes") {
        const socketMatch = key.match(/(AM\d|LGA\s*\d+|LGA\s*115x|LGA\s*1200|LGA\s*1700|LGA\s*1851)/i);
        if (socketMatch) {
...
export function normalizeCpuCooler(rawSpecs: Record<string, string>, productTitle?: string): CpuCoolerSpecs {
  ...
  // Noise level
  const noiseLevel = rawSpecs.noise_level || rawSpecs.nivel_de_ruido || rawSpecs.ruido || "Desconocido";

  const heatpipesRaw = rawSpecs.has_heatpipes || rawSpecs["¿heatpipes?"] || rawSpecs.heatpipes || "";
  const hasHeatpipes =
  ...
  if (compatibleSockets.length === 0) {
    const socketsRaw = rawSpecs.sockets_compatibles || rawSpecs.compatible_sockets || rawSpecs.sockets || "";
    if (socketsRaw) {
