...
export function normalizeCpuCooler(
	...
	let fanSize =
		rawSpecs.fan_size ||
		rawSpecs.tamaño_ventilador ||
		rawSpecs.tamano_ventilador ||
		"";
	if (!fanSize && title) {
	...
	// Noise level
	const noiseLevel =
		rawSpecs.noise_level ||
		rawSpecs.nivel_de_ruido ||
		rawSpecs.ruido ||
		"Desconocido";

	const heatpipesRaw =
		rawSpecs.has_heatpipes ||
		rawSpecs["¿heatpipes?"] ||
		rawSpecs.heatpipes ||
		"";
	const hasHeatpipes =
	...
	if (compatibleSockets.length === 0) {
		const socketsRaw =
			rawSpecs.sockets_compatibles ||
			rawSpecs.compatible_sockets ||
			rawSpecs.sockets ||
			"";
		if (socketsRaw) {
