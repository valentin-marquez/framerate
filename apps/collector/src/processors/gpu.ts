...
function parseBoolean(value?: string): boolean {
  ...
  return (
    normalized === "sí" || normalized === "si" || normalized === "yes" || normalized === "true"
  );
}
...
function buildVideoPortsFromSpDigital(rawSpecs: Record<string, string>): string[] {
  ...
  // HDMI
  const hdmiCount =
    rawSpecs["Número de puertos HDMI"] || rawSpecs["Cantidad de puertos HDMI"] || "";
...
function buildCoolingFromSpDigital(rawSpecs: Record<string, string>): string {
  ...
  const coolingTech =
    rawSpecs["Tecnología  de enfriamiento"] || rawSpecs["Tecnología de enfriamiento"] || "";
