...
function extractGpuModelFromTitle(title: string): string {
  ...
  if (chip.manufacturer === "AMD") {
    let variant = "";
    if (chip.variant) {
      variant = chip.variant === "XT" ? " XT" : chip.variant === "XTX" ? " XTX" : "";
    }
...
function extractManufacturerFromTitle(title: string): string {
  ...
  if (titleUpper.includes("RADEON") || titleUpper.match(/\bRX\s*\d{4}\b/) || titleUpper.match(/\bRX\s*\d{3}\b/)) {
    return "AMD";
  }
 ...
 */
function buildVideoPortsFromSpDigital(rawSpecs: Record<string, string>): string[] {
  const ports: string[] = [];

  // HDMI
  const hdmiCount = rawSpecs["Número de puertos HDMI"] || rawSpecs["Cantidad de puertos HDMI"] || "";
  ...
  // DisplayPort
  const dpCount = rawSpecs["Cantidad de DisplayPorts"] || rawSpecs["Número de DisplayPorts"] || "";
...
function buildCoolingFromSpDigital(rawSpecs: Record<string, string>): string {
  ...
  const coolingTech = rawSpecs["Tecnología  de enfriamiento"] || rawSpecs["Tecnología de enfriamiento"] || "";
 ...
 */
function buildFrequenciesFromSpDigital(rawSpecs: Record<string, string>): string {
  const boostClock = rawSpecs["Aumento de la velocidad de reloj del procesador"] || "";
...
export const GpuProcessor = {
  normalize(rawSpecs: Record<string, string>, title = ""): GpuSpecs {
    ...
    const isPcExpress =
      rawSpecs.Fabricante !== undefined || rawSpecs.GPU !== undefined || rawSpecs.Memoria !== undefined;
   ...
   */
  normalizeSpDigital(rawSpecs: Record<string, string>, title: string): GpuSpecs {
    // Fabricante de GPU (NVIDIA, AMD, Intel)
    const spManufacturer = rawSpecs["Familia de procesadores de gráficos"] || "";
    ...
    const memory = buildMemoryFromSpDigital(rawSpecs) || extractMemoryFromTitle(title);
    ...
    const memoryFrequency = rawSpecs["Velocidad de transferencia de datos"] || "";
    ...
    // Conectores de poder
    const powerConnectorsRaw = rawSpecs["Conectores de energia suplementario"] || "";
    const powerConnectors = powerConnectorsRaw ? parseList(powerConnectorsRaw.replace(/\s*,\s*/g, "\n")) : [];

    // Puertos de video
    const videoPorts = buildVideoPortsFromSpDigital(rawSpecs);

    // Revisar edición OC
    const isOc = parseBoolean(rawSpecs["Edición sobreacelerada (OC, Overclocked)"]);

    return {
      manufacturer,
      gpu_model: isOc && gpuModel && !gpuModel.includes("OC") ? `${gpuModel} OC` : gpuModel,
      memory,
      bus,
      frequencies,
