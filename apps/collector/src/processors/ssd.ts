...
function extractFormatFromTitle(title: string): string {
  ...
  if (
    ...
    (titleUpper.includes("SOLIDO") && !titleUpper.includes("M.2") && !titleUpper.includes("NVME"))
  ) {
    return '2.5"';
  }
...
function extractBusFromTitle(title: string): string {
  ...
  if (titleUpper.includes('2.5"') || titleUpper.includes("2.5''") || titleUpper.includes("SSD 2.5")) {
    return "SATA 3 (6.0 Gb/s)";
  }
...
function extractLineFromTitle(title: string): string {
  ...
  if (titleUpper.includes("SU650") || titleUpper.includes("ULTIMATE SU650")) return "ADATA Ultimate SU650";
...
export const SsdProcessor = {
  normalize(rawSpecs: Record<string, string>, title = ""): SsdSpecs {
    ...
    if (spDigitalInterface) {
      if (spDigitalInterface.includes("5.0") || spDigitalInterface.includes("5")) {
        busFromSpDigital = spDigitalNvme.toLowerCase() === "si" ? "PCIe 5.0 NVMe" : "PCIe 5.0";
      } else if (spDigitalInterface.includes("4.0") || spDigitalInterface.includes("4")) {
        busFromSpDigital = spDigitalNvme.toLowerCase() === "si" ? "PCIe 4.0 NVMe" : "PCIe 4.0";
      } else if (spDigitalInterface.includes("3.0") || spDigitalInterface.includes("3")) {
        busFromSpDigital = spDigitalNvme.toLowerCase() === "si" ? "PCIe 3.0 NVMe" : "PCIe 3.0";
    ...
    const spDigitalFormFactor =
      rawSpecs["Factor de forma de disco SSD"] || rawSpecs["factor de forma de disco ssd"] || "";
    const spDigitalM2Size = rawSpecs["Tamaño de la unidad SSD M.2"] || rawSpecs["tamaño de la unidad ssd m.2"] || "";
    ...
    } else if (spDigitalFormFactor.includes("2.5") || spDigitalFormFactor.includes("2,5")) {
      formatFromSpDigital = '2.5"';
    }

    // SP Digital: "Tipo de memoria" -> nand_type (e.g., "3D TLC")
    const spDigitalNandType = rawSpecs["Tipo de memoria"] || rawSpecs["tipo de memoria"] || "";

    // SP Digital: "SDD, capacidad" -> capacity (e.g., "2,05 TB")
    const spDigitalCapacity = rawSpecs["SDD, capacidad"] || rawSpecs["sdd, capacidad"] || "";
    ...
    if (spDigitalCapacity) {
      ...
      if (tbMatch) {
        ...
        capacityFromSpDigital = tb >= 1 ? `${Math.round(tb)} TB` : `${Math.round(tb * 1000)} GB`;
      } else if (gbMatch) {
    ...
    const spDigitalReadSpeed = rawSpecs["Velocidad de lectura"] || rawSpecs["velocidad de lectura"] || "";
    const spDigitalWriteSpeed = rawSpecs["Velocidad de escritura"] || rawSpecs["velocidad de escritura"] || "";
    ...
    if ((finalBus === "NVMe" || finalBus === "PCIe NVMe" || !finalBus) && finalReadSpeed) {
    ...
    return {
      ...
      capacity: rawSpecs.capacity || rawSpecs.capacidad || capacityFromSpDigital || titleCapacity || "",
      format: rawSpecs.format || rawSpecs.formato || formatFromSpDigital || titleFormat || "",
      bus: finalBus,
      has_dram: hasDram,
      nand_type:
        rawSpecs.nand_type || rawSpecs.tipo_memoria_nand || rawSpecs["tipo memoria nand"] || spDigitalNandType || "",
