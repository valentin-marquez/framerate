...
function parseCoolerIncluded(value?: string): boolean {
  ...
  if (normalized === "no" || normalized === "no posee" || normalized === "false" || normalized === "") {
    return false;
  }
...
export const CpuProcessor = {
  normalize(rawSpecs: Record<string, string>): CpuSpecs {
    const frequency = rawSpecs.frequency || rawSpecs.frecuencia || rawSpecs["frecuencia del procesador"] || "";
    ...
    const cores = rawSpecs.cores || rawSpecs.núcleos || rawSpecs["número de núcleos de procesador"] || "";
    const threads = rawSpecs.threads || rawSpecs.hilos || rawSpecs["número de hilos de ejecución"] || "";
    ...
    return {
      manufacturer: rawSpecs.manufacturer || rawSpecs.fabricante || rawSpecs["familia de procesador"] || "",
      frequency,
      frequency_turbo: frequencyTurbo,
      cores_threads: coresThreads,
      cache: rawSpecs.cache || rawSpecs.caché || rawSpecs["memoria caché"] || rawSpecs["caché del procesador"] || "",
      socket: rawSpecs.socket || rawSpecs.enchufe || rawSpecs.zócalo || rawSpecs["socket de procesador"] || "",
      ...
      manufacturing_process:
        ...
        "",
      tdp: rawSpecs.tdp || rawSpecs.consumo || rawSpecs["potencia de diseño térmico (tdp)"] || "",
