...
function extractIlluminationFromTitle(title: string): string {
  ...
  if (titleUpper.includes("ARGB") || titleUpper.includes("A-RGB") || titleUpper.includes("ADDRESSABLE RGB")) {
    return "ARGB";
  }
...
export function normalizeCaseFan(rawSpecs: Record<string, string>, productTitle?: string): CaseFanSpecs {
  const title = productTitle || "";

  const powerConnectors: string[] = [];
  const powerRaw = rawSpecs.power_connectors || rawSpecs.conexiones_de_poder || rawSpecs.conectores || "";
  if (powerRaw) {
  ...
  let fansIncluded =
    rawSpecs.fans_included || rawSpecs.included_fans || rawSpecs.ventiladores_incluidos || rawSpecs.cantidad || "";
  ...
  const staticPressure = rawSpecs.static_pressure || rawSpecs.presión_estática || rawSpecs.presion_estatica || "";

  const noiseLevel = rawSpecs.noise_level || rawSpecs.nivel_de_ruido || rawSpecs.ruido || "";

  let lightingControl =
    rawSpecs.lighting_control || rawSpecs.control_de_iluminación || rawSpecs.control_iluminacion || "";
