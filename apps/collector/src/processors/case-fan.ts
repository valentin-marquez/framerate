...
export function normalizeCaseFan(
	...
	let illumination =
		rawSpecs.illumination || rawSpecs.iluminación || rawSpecs.iluminacion || "";
	if (
		illumination.toLowerCase() === "no" ||
		illumination.toLowerCase() === "sin iluminación"
	) {
		illumination = "No";
	...
	const hubRaw =
		rawSpecs.includes_hub ||
		rawSpecs["¿incluye_hub?"] ||
		rawSpecs.incluye_hub ||
		"";
	const includesHub =
	...
	const bearing =
		rawSpecs.bearing || rawSpecs.rodamiento || rawSpecs.tipo_rodamiento || "";

	const airflow =
		rawSpecs.airflow || rawSpecs.flujo_de_aire || rawSpecs.flujo_aire || "";

	const staticPressure =
		rawSpecs.static_pressure ||
		rawSpecs.presión_estática ||
		rawSpecs.presion_estatica ||
		"";

	const noiseLevel =
		rawSpecs.noise_level || rawSpecs.nivel_de_ruido || rawSpecs.ruido || "";
	...
	let lightingControl =
		...
		"";
	if (
		lightingControl.toLowerCase() === "no posee" ||
		lightingControl.toLowerCase() === "no"
	) {
