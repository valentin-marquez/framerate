...
function extractPfcFromName(name: string): boolean {
	...
	return (
		normalized.includes("PFC ACTIV") ||
		normalized.includes("PFC ACTIVA") ||
		normalized.includes("ACTIVE PFC")
	);
}
...
export const PsuProcessor = {
	normalize(rawSpecs: Record<string, string>, productName?: string): PsuSpecs {
		...
		let wattage =
			rawSpecs.wattage ||
			rawSpecs.potencia ||
			rawSpecs.potencia_de_salida ||
			rawSpecs["potencia de salida"] ||
			"";
		...
		let certification =
			rawSpecs.certification ||
			rawSpecs.certificación ||
			rawSpecs.certificacion ||
			"";
		...
		const rail12v =
			rawSpecs.rail_12v ||
			rawSpecs.corriente_en_la_línea_de_12_v ||
			rawSpecs.corriente_12v ||
			"";

		const rail5v =
			rawSpecs.rail_5v ||
			rawSpecs.corriente_en_la_línea_de_5_v ||
			rawSpecs.corriente_5v ||
			"";

		const rail3v3 =
			rawSpecs.rail_3v3 ||
			rawSpecs["corriente_en_la_línea_de_3.3_v"] ||
			rawSpecs["corriente_3.3v"] ||
			"";
