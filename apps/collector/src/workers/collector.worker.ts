...
type CategorySlug = "gpu" | "cpu" | "psu" | "motherboard" | "case" | "ram" | "hdd" | "ssd" | "case_fan" | "cpu_cooler";
...
async function getOrCreateCategory(slug: CategorySlug): Promise<string | null> {
  ...
  const { data: existing } = await supabase.from("categories").select("id").eq("slug", config.slug).single();
...
async function getOrCreateBrandInternal(normalizedName: string, slug: string): Promise<string | null> {
...
async function upsertListing(product: ProductData, productId: string, storeId: string): Promise<string | null> {
...
async function recordPriceHistory(listingId: string, priceCash: number, priceNormal: number): Promise<void> {
...
async function normalizeSpecs(
  ...
  if (category === "hdd" && mpn) {
    const specs = await hddIAExtractor.extract(mpn, `Title: ${title}\nSpecs: ${JSON.stringify(rawSpecs)}`);
    if (specs) return specs;
  }

  if (category === "ssd" && mpn) {
    const specs = await ssdIAExtractor.extract(mpn, `Title: ${title}\nSpecs: ${JSON.stringify(rawSpecs)}`);
    if (specs) return specs;
  }

  if (category === "ram" && mpn) {
    const specs = await ramIAExtractor.extract(mpn, `Title: ${title}\nSpecs: ${JSON.stringify(rawSpecs)}`);
    if (specs) return specs;
  }

  if (category === "motherboard" && mpn) {
    const specs = await motherboardIAExtractor.extract(mpn, `Title: ${title}\nSpecs: ${JSON.stringify(rawSpecs)}`);
    if (specs) return specs;
  }
  if (category === "gpu" && mpn) {
    const specs = await gpuIAExtractor.extract(mpn, `Title: ${title}\nSpecs: ${JSON.stringify(rawSpecs)}`);
    if (specs) return specs;
  }

  if (category === "cpu" && mpn) {
    const specs = await cpuIAExtractor.extract(mpn, `Title: ${title}\nSpecs: ${JSON.stringify(rawSpecs)}`);
    if (specs) return specs;
  }

  if (category === "psu" && mpn) {
    const specs = await psuIAExtractor.extract(mpn, `Title: ${title}\nSpecs: ${JSON.stringify(rawSpecs)}`);
    if (specs) return specs;
  }

  if (category === "case_fan" && mpn) {
    const specs = await caseFanIAExtractor.extract(mpn, `Title: ${title}\nSpecs: ${JSON.stringify(rawSpecs)}`);
    if (specs) return specs;
  }

  if (category === "case" && mpn) {
    const specs = await caseIAExtractor.extract(mpn, `Title: ${title}\nSpecs: ${JSON.stringify(rawSpecs)}`);
    if (specs) return specs;
  }

  if (category === "cpu_cooler" && mpn) {
    const specs = await cpuCoolerIAExtractor.extract(mpn, `Title: ${title}\nSpecs: ${JSON.stringify(rawSpecs)}`);
    if (specs) return specs;
  }
...
async function processProductImage(mpn: string | undefined, imageUrl: string | undefined): Promise<string | null> {
...
async function processProduct(
  ...
  if (!productId) {
    productId = await createProduct(product, category, categoryId, brandId, normalizedSpecs, imageUrl);
...
async function processApiCategory(
  ...
  for (const rawProduct of products) {
    const product = transformProduct ? transformProduct(rawProduct) : rawProduct;
    const result = await processProduct(product, category as CategorySlug, categoryId, storeId, crawlerType);
    if (result.success) processedCount++;
    else if (result.error) logger.warn(`[API][${category}] ${result.error}: ${product.url?.slice(-50)}`);
  }

  const duration = ((Date.now() - startTime) / 1000).toFixed(1);
  logger.info(`=== [API] ${category} completed: ${processedCount} products in ${duration}s ===`);
...
async function handleMyShopJob(job: CollectorJobData): Promise<JobResult> {
  const startTime = Date.now();
  ...
  const categoriesToProcess: MyShopCategory[] =
    job.category === "all" || !job.category
  ...
  const duration = (Date.now() - startTime) / 1000;
  logger.info(`[MyShop] Job completed: ${totalProcessed} products in ${duration.toFixed(1)}s`);
  ...
  try {
    ...
    switch (job.crawler) {
      ...
      case "sp-digital":
        ...
        break;
      case "myshop":
        result = await handleMyShopJob(job);
        break;
      default:
