...
const PC_EXPRESS_VALIDATION_RULES: Partial<
	Record<CategorySlug, ValidationRule[]>
> = {
	motherboard: [
		{
	...
	case_fan: [
		{
			check: (title) => {
				const hasVentilador =
					title.includes("VENTILADOR") || title.includes("VENTILADORES");
				const isExcluded =
...
async function processProduct(
	...
	const normalizedSpecs = await normalizeSpecs(
		category,
		rawSpecs,
		product.title,
		product.mpn,
...
async function processCategory<T extends string>(
	...
	for (let i = 0; i < productUrls.length; i += BATCH_SIZE) {
		...
		logger.info(
			`[${category}] Batch ${batchNumber}/${totalBatches} (${batch.length} products)`,
		);
	...
	logger.info(
		`=== ${category} completed: ${processedCount} products in ${duration}s ===`,
	);
