...
async function downloadImage(imageUrl: string): Promise<{
	...
	try {
		...
		if (mimeType === "image/png") {
			...
			if (
				urlExtension &&
				["png", "jpeg", "jpg", "webp"].includes(urlExtension)
			) {
...
export async function uploadProductImage(
	...
	if (imageData.data.byteLength > maxSize) {
		logger.info(
			`Image for MPN ${mpn} exceeds ${maxSize} bytes (${imageData.data.byteLength}), compressing...`,
		);

		try {
			const compressed = await sharp(Buffer.from(imageData.data))
				.webp({ quality: 80 })
				.toBuffer();
			...
			logger.info(
				`Compressed image for MPN ${mpn}: ${imageData.data.byteLength} -> ${finalData.byteLength} bytes`,
			);
		} catch (compressError) {
			logger.error(
				`Failed to compress image for MPN ${mpn}:`,
	...
	const filePath = getProductImagePath(
		sanitizedMpn,
		finalExtension as "png" | "jpeg" | "webp",
	);
	...
	if (uploadError) {
		if (
			...
			const existingUrl = getStoragePublicUrl(
				SUPABASE_URL,
				StorageBuckets.PRODUCT_IMAGES,
				filePath,
			);
