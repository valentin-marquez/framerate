...
describe("GPU Title Normalization", () => {
  test("removes Part Numbers (P/N)", () => {
    ...
    expect(normalizeGpuTitle("BRIDGE EVGA NVIDIA NVLINK 3 SLOT P/N 100-2W-0029-LR")).toBe(
      "BRIDGE EVGA NVIDIA NVLINK 3 SLOT",
    );
    ...
    expect(normalizeGpuTitle("TARJETA DE VIDEO GIGABYTE RADEON 9060XT GAMING 16GB P/N GV-R9060XTGAMING-16GD")).toBe(
      "Gigabyte Gaming RX 9060 XT 16GB",
    );
    expect(
      normalizeGpuTitle("TARJETA DE VIDEO GIGABYTE RADEON 9060XT GAMING OC 16GB P/N GV-R9060XTGAMING OC 16GD"),
  ...
  test("handles missing VRAM specification", () => {
    ...
    expect(normalizeGpuTitle("MSI Ventus RTX 5060 Ti OC")).toBe("MSI Ventus RTX 5060 Ti OC");
  });
  ...
  test("handles missing brand", () => {
    ...
    expect(normalizeGpuTitle("GT 710 2GB Low Profile", undefined, "ASUS")).toBe("ASUS GT 710 2GB Low Profile");
  });

  test("preserves correct formatting", () => {
    expect(normalizeGpuTitle("Zotac AMP Infinity RTX 5090 32GB")).toBe("Zotac AMP Infinity RTX 5090 32GB");
    expect(normalizeGpuTitle("ASUS Dual RTX 5060 Ti 16GB OC")).toBe("ASUS Dual RTX 5060 Ti 16GB OC");
    ...
    expect(normalizeGpuTitle("ASUS ROG Astral RTX 5080 16GB OC")).toBe("ASUS ROG Astral RTX 5080 16GB OC");
  });
});
