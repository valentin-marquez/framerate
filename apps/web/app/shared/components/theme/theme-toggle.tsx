...
import { useOptimisticThemeMode } from "~/shared/lib/client";
    ...
    <div className="flex items-center gap-1 rounded-full border border-border bg-background p-1">
      {options.map(({ id, icon: Icon, color, label }) => (
        <button
          ...
          type="button"
          className={cn(
            "relative flex size-8 items-center justify-center rounded-full text-muted-foreground transition-colors hover:text-foreground cursor-pointer focus-visible:outline-none",
        ...
        >
          ...
          <Icon className={cn("relative z-10 size-4", theme === id ? color : "text-muted-foreground")} />
        </button>
      ))}
    </div>
  );
