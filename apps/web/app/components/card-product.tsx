...
export function CardProduct({ product, className }: CardProductProps) {
  const { id, name = "Producto sin nombre", image_url, prices, brand, slug, popularity_score } = product;
      ...
      <div className="nested-p-4 flex flex-1 flex-col p-4 pt-2">
        <div className="flex mb-2 items-center justify-between gap-2">
          <div className="flex items-center gap-2">
            <Badge variant="outline" className="text-[10px] px-2 py-0.5 h-5 border-primary/20 bg-accent text-primary">
              {brand?.name || "Genérico"}
            </Badge>
            {popularity_score > 0 && (
        ...
        <div className="mt-auto pt-3 border-t border-border/50">
          <div className="mb-3 grid grid-cols-2 gap-2">
            <div className="flex flex-col">
              <span className="text-[10px] text-muted-foreground font-medium uppercase">Efectivo</span>
              <span className="text-lg font-bold text-primary tracking-tight leading-none">{formattedCashPrice}</span>
            </div>
            {formattedNormalPrice && (
              <div className="flex flex-col items-end border-l border-border/50 pl-2">
                <span className="text-[10px] text-muted-foreground font-medium uppercase">Normal</span>
